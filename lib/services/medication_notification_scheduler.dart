...
class MedicationNotificationScheduler {
  ...
  Future<void> scheduleAllNotifications(String userId) async {
    ...
    try {
      ...
      for (var row in response) {
        ...
        final nombreMedicamento = itemData['item_nomb']?.toString() ?? 'Medicamento';
  ...
  Future<void> rescheduleForSeguimiento(String idSeguimiento) async {
    try {
      ...
      if (response != null) {
        ...
        print('🔄 Notificaciones reprogramadas para seguimiento: $idSeguimiento');
      }
    } catch (e) {
  ...
  // Reprogramar cuando se elimina un seguimiento
